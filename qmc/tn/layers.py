...
class MPSLayer(tf.keras.layers.Layer):
    def __init__(self, n_sites, d_bond, d_phys, n_output):
        ...
        mps = [
            ...
            self._boundary(self.d_phys, self.d_bond),
            self._middle(self.d_phys, self.single_rank - 1, self.d_bond),
            self._center(self.num_classes, self.d_bond)
        ]
    ...
    def call(self, inputs, **kwargs):
        def f(input_vec, mps_var):
            ...
            # Contract
            ans = tn.contractors.greedy(tn.reachable(cn))
            return tf.nn.softmax(ans.tensor)
    ...
    @staticmethod
    def _center(output, bond):
        return np.random.normal(0, 1e-2, size=(output, bond, bond))
